...
from analyzers.scorer import score_resume
...
def run_cli(args_resume_file: str, args_job_desc: str, args_keywords: str):
    ...
    target_keywords = [kw.strip().lower() for kw in args_keywords.split(",") if kw.strip()]
    ...
    scores = score_resume(parsed, job_desc, target_keywords)
    if "error" in scores:
    ...
    print("="*50)
    print(f"Name: {parsed['name'] or 'Not found'}")
    print(f"Email(s): {', '.join(parsed['email']) or 'Not found'}")
    print(f"Phone(s): {', '.join(parsed['phone']) or 'Not found'}")
    print(f"Skills: {', '.join(parsed['skills'][:5])}{'...' if len(parsed['skills']) > 5 else ''}")
    ...
    print(f"  Semantic Matching: {scores['semantic_matching']}/100")
    print(f"  Skill Relevance: {scores['skill_relevance']}/100")
...
def analyze_in_thread(gui_vars):
    ...
    gui_vars['status_label'].config(text="Analyzing... (This may take 10-30s for AI models)")
    gui_vars['progress_bar'].start()  # Start spinner
    
    try:
        resume_path = gui_vars['resume_path'].get()
        job_desc_text = gui_vars['job_desc_text'].get("1.0", tk.END).strip()
        use_keywords = gui_vars['use_keywords_var'].get()
        ...
        target_keywords = [kw.strip().lower() for kw in "python, java, sql, git".split(",") if kw.strip()]  # Defaults
        if use_keywords:
            job_desc = ""
        else:
        ...
        # Format output for GUI (enhanced with emojis for modern feel)
        summary = "🚀 AI RESUME ANALYSIS SUMMARY\n"
        summary += "="*60 + "\n\n"
        summary += f"👤 Name: {parsed['name'] or 'Not found'}\n"
        summary += f"📧 Email(s): {', '.join(parsed['email']) or 'Not found'}\n"
        summary += f"📱 Phone(s): {', '.join(parsed['phone']) or 'Not found'}\n"
        summary += f"🛠️  Skills: {', '.join(parsed['skills'][:5])}{'...' if len(parsed['skills']) > 5 else ''}\n"
        summary += f"🎓 Education: {len(parsed['education'])} entries\n"
        summary += f"💼 Experience: {len(parsed['experience'])} entries\n\n"
        summary += "📊 SCORES:\n"
        summary += f"  Completeness: {scores['completeness']}/100\n"
        summary += f"  Semantic Matching: {scores['semantic_matching']}/100\n"
        summary += f"  Skill Relevance: {scores['skill_relevance']}/100\n"
        summary += f"  Total Score: {scores['total']}/100 ⭐\n"
        summary += "="*60 + "\n"
        ...
        gui_vars['status_label'].config(text=f"Analysis complete at {time.strftime('%H:%M:%S')}! Use 'Save JSON' to export.")
        gui_vars['output_data'] = output_data
        
    except Exception as e:
        messagebox.showerror("Analysis Error", str(e))
        gui_vars['status_label'].config(text=f"Error at {time.strftime('%H:%M:%S')}: {str(e)[:50]}...")
    finally:
...
def launch_gui():
    ...
    root.geometry("800x850")  # Bigger starting size
    ...
    tk.Label(header_frame, text="🤖 AI Resume Analyzer", font=('Segoe UI', 18, 'bold'), 
             fg="white", bg="#007BFF").pack(expand=True)
    ...
    # Resume Upload Card
    upload_card = tk.LabelFrame(main_frame, text="📁 Upload Resume", font=('Segoe UI', 12, 'bold'), 
                                bg="white", relief=tk.RIDGE, bd=1, padx=15, pady=10)
    upload_card.pack(fill=tk.X, pady=(0, 15))
    gui_vars['resume_path'] = tk.StringVar()
    ttk.Entry(upload_card, textvariable=gui_vars['resume_path'], width=60, state="readonly").pack(pady=5)
    ttk.Button(upload_card, text="Browse PDF", 
               command=lambda: gui_vars['resume_path'].set(
                   filedialog.askopenfilename(title="Select PDF Resume", filetypes=[("PDF files", "*.pdf")])
               )).pack(pady=5)
    
    # Job Description Card
    job_card = tk.LabelFrame(main_frame, text="📝 Job Description (Optional for AI Scoring)", 
                             font=('Segoe UI', 12, 'bold'), bg="white", relief=tk.RIDGE, bd=1, padx=15, pady=10)
    job_card.pack(fill=tk.X, pady=(0, 15))
    gui_vars['job_desc_text'] = scrolledtext.ScrolledText(job_card, height=4, width=70, font=('Segoe UI', 10))
    gui_vars['job_desc_text'].pack(pady=5)
    gui_vars['use_keywords_var'] = tk.BooleanVar(value=True)
    ttk.Checkbutton(job_card, text="Use default keywords (python, java, sql, git)", 
                    variable=gui_vars['use_keywords_var']).pack(pady=5)
    ...
    # Use tk.Button for hover effects (ttk doesn't support background configure)
    analyze_btn = tk.Button(button_frame, text="🚀 Analyze Resume", command=start_analysis,
                            bg="#007BFF", fg="white", font=('Segoe UI', 10, 'bold'), padx=20, pady=8, relief=tk.FLAT)
    analyze_btn.pack(side=tk.LEFT, padx=(0, 10))
    
    save_btn = tk.Button(button_frame, text="💾 Save JSON", command=lambda: save_json(gui_vars),
                         bg="#28a745", fg="white", font=('Segoe UI', 10, 'bold'), padx=20, pady=8, relief=tk.FLAT)
    save_btn.pack(side=tk.LEFT, padx=(0, 10))
    
    clear_btn = tk.Button(button_frame, text="🗑️ Clear Results", command=lambda: clear_results(gui_vars),
                          bg="#dc3545", fg="white", font=('Segoe UI', 10, 'bold'), padx=20, pady=8, relief=tk.FLAT)
    ...
    # Results Card (Bigger!)
    results_card = tk.LabelFrame(main_frame, text="📊 Analysis Results", 
                                 font=('Segoe UI', 12, 'bold'), bg="white", relief=tk.RIDGE, bd=1, padx=15, pady=10)
    results_card.pack(fill=tk.BOTH, expand=True, pady=(0, 10))
    gui_vars['output_text'] = scrolledtext.ScrolledText(results_card, height=25, width=80, font=('Segoe UI', 10), 
                                                        bg="#f9f9f9", relief=tk.SUNKEN, bd=1)
    gui_vars['output_text'].pack(fill=tk.BOTH, expand=True)
    
    # Status Bar
    gui_vars['status_label'] = tk.Label(main_frame, text="Ready to upload and analyze! 👋", 
                                        relief=tk.SUNKEN, anchor=tk.W, bg="#e9ecef", padx=10, pady=5, 
                                        font=('Segoe UI', 9))
...
def main():
    ...
    parser.add_argument("--keywords", default="python, java, sql, git", 
                        help="Fallback keywords (comma-separated)")
    args = parser.parse_args()
    
    if args.resume_file is None:
        launch_gui()
    else:
        run_cli(args.resume_file, args_job_desc=args.job_desc, args_keywords=args.keywords)
